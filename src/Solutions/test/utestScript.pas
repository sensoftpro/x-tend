...
function TtestScript.CheckCanChangeField(const AView: TView; const AEntity: TEntity;
  const AFieldName: string; const ANewValue: Variant; var AHandled: Boolean): Boolean;
begin
  Result := True;
end;

procedure TtestScript.DoAfterEntityCreation(const AHolder: TChangeHolder; const AOwnerContext: TObject; const AEntity: TEntity);
var
...
function TtestScript.DoCheckActionFlags(const AView: TView; const AActionName: string;
...
procedure TtestScript.DoCreateDefaultEntities(const ADomain: TObject; const AHolder: TChangeHolder);
var
...
procedure TtestScript.DoCreateDefinitions;
begin
  inherited DoCreateDefinitions;
end;

function TtestScript.DoExecuteAction(const AView: TView; const AActionName: string;
...
function TtestScript.GetFullText(const AEntity: TEntity; var AHandled: Boolean): string;
var
  vCollectionName: string;
begin
  vCollectionName := AEntity.CollectionName;
...
RegisterScript('test', TtestScript);
